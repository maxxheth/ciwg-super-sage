...
Use TLS connection string `tls://redis:6379` instead of TCP connection `tcp://redis:6379` string in order to enable transport level security.

```php
require 'Credis/Client.php';
$redis = new Credis_Client('tls://redis:6379');
$redis->set('awesome', 'absolutely');
echo sprintf('Is Credis awesome? %s.\n', $redis->get('awesome'));
...
$cluster = new Credis_Cluster(array(
    array('host' => 'redis', 'port' => 6379, 'alias'=>'alpha'),
    array('host' => 'redis', 'port' => 6380, 'alias'=>'beta')
));
...
$cluster = new Credis_Cluster(array(
    array('host' => 'redis', 'port' => 6379, 'alias'=>'master', 'master'=>true),
    array('host' => 'redis', 'port' => 6380, 'alias'=>'slave')
));
$cluster->set('key','value');
...
$sentinel = new Credis_Sentinel(new Credis_Client('redis',26379));
