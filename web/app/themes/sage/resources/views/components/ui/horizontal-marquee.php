...
<div class="relative w-full {{ $maxWidth }} mx-auto overflow-hidden bg-white bg-opacity-75 py-4">
    <div class="marquee flex items-center {{ $gap }}" style="will-change: transform; animation: marquee-scroll {{ $animationDuration }}s linear infinite;">
        @foreach ($logos as $logo)
            <div class="flex-shrink-0 transition-all cursor-pointer duration-500 ease-in-out hover:scale-125">
                <img src="{{ $logo }}" alt="Partner Logo" class="h-12 w-auto select-none pointer-events-auto" draggable="false" />
            </div>
        @endforeach
